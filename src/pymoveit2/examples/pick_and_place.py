...
ros2 run pymoveit2 pick_and_place.py --ros-args -p target_color:= RED -p target_shape = SPHERE
ros2 run pymoveit2 pick_and_place.py --ros-args -p target_color:=BLUE -p target_shape = SPHERE
ros2 run pymoveit2 pick_and_place.py --ros-args -p target_color:= RED -p target_shape = CUBE
ros2 run pymoveit2 pick_and_place.py --ros-args -p target_color:=BLUE -p target_shape = CUBE

sometimes quat_xyzw1 needs to be changed manually, if trajectory can not be calculated.

working on to make it more robust for any coordinates and orientation.
